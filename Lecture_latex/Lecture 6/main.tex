...
{Orthogonality and Applications of Linear Algebra}
...
Now, can you tell me why $A^TA\hat{x} =A^Tb$ must be consistent? The geometric reason behind: $Ax=p$ must have solutions.

\vspace{3pt}
How to prove? Hint: $N(A^T A) = N(A)$ introduced on Review Class.
...
\begin{frame}{Linear Algebra in Graphs}
Every single graph can be represented by a matrix.
...
\end{figure}
\vspace{-2pt}
Eliminate it to RREF. Find the nullspace solution.

\begin{center}
    \# of nodes $- 1 = r$
\end{center}

Dependent rows represent there exists a loop.

\begin{center}
    \# of loops$ = dim(N(A^T))$
\end{center}

We can get the Euler's Formula (holds for all graphs):
\begin{center}
    $dim(N(A^T)) = m - r \rightarrow$ \# of loops = \# of edges - (\# of nodes - 1)
\end{center}

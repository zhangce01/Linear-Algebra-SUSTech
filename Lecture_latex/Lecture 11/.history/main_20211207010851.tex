...
For real matrix, a matrix is orthogonal if $A^TA=AA^T=I$, so for the complex matrix, a matrix is unitary if $A^HA=AA^H=I$.
...
For Hermitian matrix (real symmetric included), it must have a unitary (orthogonal included) matrix to diagonlize.
\begin{equation*}
    A=U\Lambda U^T
\end{equation*}

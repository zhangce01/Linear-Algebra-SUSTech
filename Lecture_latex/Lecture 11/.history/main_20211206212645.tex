...
\vspace{5pt}
If a matrix is diagonlizable, we can easily find the diagonalization matrix! But the question is, we cannot tell if a matric has $n$ linearly independent eigenvectors (i.e. if it is diagonlizable).
\end{frame}
...
\vspace{3pt}
Why? I will not give you proof, but I want you to understand. I will show from 2 sides:

\begin{enumerate}
    \item For each eigenvalue, there must be at least 1 eigenvector (authentic to say: 1-dimensional).
    \item For the same eigenvector, it cannot correspond to 2 eigenvalues.
\end{enumerate}

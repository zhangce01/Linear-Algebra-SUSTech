...
%---------------------------------------------------------
\section{Some Issues}
\begin{frame}{A Summary of Peer Supporting Class (First Half Semester)}
For the Linear Algebra course, we 4 tutors hold 20 classes in total, and 468 people participate this program. Thank you all for participating! And the Study Center of SUSTech will send you a small gift if you attend 80\% or more classes in this program.

\vspace{3pt}
A total of 312 people attend the previous 6 lectures on Tuesday.
\begin{figure}
    \centering
    \includegraphics[width=0.7\textwidth]{psc.jpg}
\end{figure}
\end{frame}

\begin{frame}{The Slides}
You may find that the slides I gave you are always pdf format (not ppt). The reason is I generate and create the slides by \LaTeX ! Try to learn it in your holiday and it will benefit you definitely.

\begin{figure}
    \centering
    \includegraphics[width=0.85\textwidth]{latex.png}
\end{figure}
\end{frame}

\begin{frame}{About the Midterm...}
First of all, from my perspective, the midterm this year is more difficult than 2019, 2020. And it only has 100 marks in total.

\vspace{3pt}
One clear feature of this exam is there are no such easy problems any more. Every problem, you need to think for a while... But I would like to say the final problem is easier than 2019, 2020, and actually, we have predicted many of the problems correctly in the review class!

\vspace{3pt}
Keep fighting if you have done a good job in the exam, and damn it if you get not so well. The marks will not determine your life!
\end{frame}
...
Then we can further extend this definition to subspaces, if all the vectors in subspace $A$ are orthogonal to all the vectors in subspace $B$, then subspaces $A$ and $B$ are orthogonal.
...
\begin{frame}{Projection Matrix}
    \begin{equation*}
        \mathbf{a}^T\mathbf{e}=\mathbf{a}^T\left( \mathbf{b}-x\mathbf{a} \right) =0\Rightarrow x=\frac{\mathbf{a}^T\mathbf{b}}{\mathbf{a}^T\mathbf{a}}\Rightarrow \mathbf{p}=\mathbf{a}\frac{\mathbf{a}^T\mathbf{b}}{\mathbf{a}^T\mathbf{a}}
    \end{equation*}
Can you give a projection matrix that can project every vector $b$ onto the line where $a$ lies in?
\begin{equation*}
    \mathbf{p}=P\mathbf{b}\Rightarrow P=\frac{\mathbf{aa}^T}{\mathbf{a}^T\mathbf{a}}
\end{equation*}
That is the matrix for projection onto line $a$. Use your knowledge of previous knowledge, $\mathbf{aa}^T$ is a scalar, vector or matrix? What about $\mathbf{a}^T\mathbf{a}$?
...
Linear Algebra is definitely a mathematical subject, but it isn't in the Advanced Mathematics (Calculus) course. That is because linear algebra is the most (not one of the most) useful course in university! The thought and proposed method in linear algebra is useful in all the subject, including chemistry, biology, electrical engineering and computer science...
...
\vspace{3pt}
Firstly, he set all the coefficients with variables $x_1,x_2,x_3,x_4$.
\begin{equation*}
...
If you multiply all the coefficients by a constant, the chemical reaction equation is still balanced, but not the simplest.
\end{frame}
